...
from spp.engine.pipeline import write_data
...
@patch('spp.engine.pipeline.write_spark_df_to_s3')
def test_spark_write_csv(write_to_s3, create_session):
    ...
    write_data(df, test_target, counter=0, logger=logger)
...
def test_spark_write_file_with_partitions(write_to_s3, create_session):
    ...
    write_data(df, test_target, counter=0, logger=logger, partitions=['_c0'])
    assert write_to_s3.call_args[0][1]['location'] == \
