...
def test_db_table_single_select():
    ...
    assert expected_query == str(Query("test", "test", "column"))


def test_db_table_list_select():
    expected_query = "SELECT column_a, column_b FROM test.test;"
    assert expected_query == str(Query("test", "test", ["column_a", "column_b"]))


def test_db_table_single_where():
    expected_query = "SELECT * FROM test.test WHERE column == 500;"
    print(str(Query("test", "test", where={"column": {"condition": "==", "value": "500"}})))
    print(expected_query)
    assert expected_query == str(Query("test", "test", where=[{"column": "column", "condition": "==", "value": "500"}]))


def test_db_table_list_where():
    expected_query = "SELECT * FROM test.test WHERE column_a > 500 AND column_b = this AND column_c < 100;"
    assert expected_query == str(Query("test", "test", where=[{"column": "column_a", "condition": ">", "value": "500"},
                                       {"column": "column_b", "condition": "=", "value": "this"},
                                       {"column": "column_c", "condition": "<", "value": "100"}]))


def test_db_table_single_select_where():
    expected_query = "SELECT column FROM test.test WHERE column = test;"
    assert expected_query == str(Query("test", "test", "column", [{"column": "column","condition": "=", "value": "test"}]))
...
def test_db_table_list_select_where():
    ...
    assert expected_query == str(Query("test", "test", ["column_a", "column_b", "column_c"],
                                   [{"column": "column_a", "condition": "=", "value": "500"},
                                    {"column": "column_b", "condition": ">", "value": "250"},
                                    {"column": "column_c", "condition": "<", "value": "500"}]))
